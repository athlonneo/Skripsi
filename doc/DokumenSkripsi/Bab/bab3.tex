...
\begin{itemize}
	\item \verb|Assignment_model| \\ Model untuk menangani \textit{database} \verb|assignments|. Fungsi yang dimiliki:
	\begin{itemize}
		\item \verb|add_assignment($id, $edit = FALSE)|\\ Menambah atau memperbarui sebuah \textit{assignment}.
		...
		\item \verb|_update_coefficients($assignment_id, $extra_time, $finish_time, $new_late_rule)| \\ Memperbarui koefisien seluruh \textit{submission} pada sebuah \textit{assignment}. Fungsi ini dipanggil oleh \verb|add_assignment($id, TRUE)|.
	\end{itemize}
	...
	\item \verb|Logs_model| \\ Model untuk menangani \textit{database} \verb|logins|. Fungsi yang dimiliki:
	\begin{itemize}
	...
	\item \verb|Notifications_model| \\ Model untuk menangani \textit{database} \verb|notifications|. Fungsi yang dimiliki:
	\begin{itemize}
	...
	\item \verb|Queue_model| \\ Model untuk menangani \textit{database} \verb|queue|. Fungsi yang dimiliki:
	\begin{itemize}
	    \item \verb|in_queue($username, $assignment, $problem)| \\ Mengembalikan \verb|TRUE| jika sebuah \textit{submission} sudah berada dalam antrian.
	    \item \verb|get_queue()| \\ Mengambil seluruh antrian.
	    \item \verb|empty_queue()| \\ Mengosongkan antrian.
	    \item \verb|add_to_queue($submit_info)| \\ Menambahkan sebuah \textit{submission} ke dalam antrian.
	    \item \verb|rejudge($assignment_id, $problem_id)| \\ Menambahkan seluruh \textit{submission} dari sebuah \textit{problem} ke dalam antrian untuk dinilai ulang.
	    \item \verb|rejudge_single($submission)| \\  Menambahkan sebuah \textit{submission} ke dalam antrian untuk dinilai ulang.
	    \item \verb|get_first_item()| \\ Mengambil \textit{entry} pertama dari antrian.
	    \item \verb|remove_item($username, $assignment, $problem, $submit_id)| \\ Menghapus sebuah \textit{entry} dari antrian.
	    \item \verb|save_judge_result_in_db ($submission, $type)| \\ Menyimpan hasil penilaian ke dalam \textit{database}. Fungsi ini dipanggil oleh \textit{controller} \verb|Queueprocess|.
	\end{itemize}
	
	\item \verb|Scoreboard_model| \\ Model untuk menangani \textit{database} \verb|scoreboard|. Fungsi yang dimiliki:
	\begin{itemize}
	    \item \verb|_generate_scoreboard($assignment_id)| \\ Membuat \textit{scoreboard} untuk sebuah \textit{assignment}. Fungsi ini dipanggil oleh \verb|update_scoreboard($assignment_id)|.
	...
	\item \verb|Settings_model| \\ Model untuk menangani \textit{database} \verb|settings|. Fungsi yang dimiliki:
	\begin{itemize}
	...
	\item \verb|Submit_model| \\ Model untuk menangani \textit{database} \verb|submissions|. Fungsi yang dimiliki:
	\begin{itemize}
	    \item \verb|get_submission($username, $assignment, $problem, $submit_id)| \\ Mengambil sebuah \textit{submission}.
	    \item \verb|get_final_submissions($assignment_id, $user_level, $username, $page_number = NULL, $filter_user = NULL, $filter_problem = NULL)| \\ Mengambil seluruh \textit{final submission} untuk sebuah \textit{assignment}.
	    \item \verb|get_all_submissions($assignment_id, $user_level, $username, $page_number = NULL, $filter_user = NULL, $filter_problem = NULL)| \\ Mengambil seluruh \textit{submission} untuk sebuah \textit{assignment}.
	    \item \verb|count_final_submissions($assignment_id, $user_level, $username, $filter_user = NULL, $filter_problem = NULL)| \\ Menghitung jumlah \textit{final submission} dari \textit{user} tertentu.
	    \item \verb|count_all_submissions($assignment_id, $user_level, $username, $filter_user = NULL, $filter_problem = NULL)| \\ Menghitung jumlah \textit{submission} dari \textit{user} tertentu.
	    \item \verb|set_final_submission($username, $assignment, $problem, $submit_id)| \\ Memperbarui sebuah \textit{submission} menjadi \textit{final}.
	...
	\item \verb|User_model| \\ Model untuk menangani \textit{database} \verb|users|. Fungsi yang dimiliki:
	\begin{itemize}
	    ...
	    \item \verb|have_email($email, $username = FALSE)| \\ Mengembalikan \verb|TRUE| jika terdapat \textit{user} selain \verb|$username| dengan email \verb|$email|.
...
\begin{itemize}
	...
	\begin{itemize}
		...
		\item \verb|pdf($assignment_id, $problem_id = NULL)| \\ Mengunduh \textit{file} PDF dari sebuah \textit{assignment}.
		\item \verb|downloadtestsdesc($assignment_id = FALSE)| \\ Mengunduh \textit{file} \textit{test case} dari sebuah \textit{assignment}.
		\item \verb|download_submissions($type = FALSE, $assignment_id = FALSE)| \\ Mengunduh seluruh \textit{file} \textit{final submission} dari sebuah \textit{assignment}.
		\item \verb|delete($assignment_id = FALSE)| \\ Menghapus sebuah \textit{assignment}.
	...
	\begin{itemize}
		...
		\item \verb|reset($passchange_key = FALSE)| \\ Memproses dan menampilkan halaman untuk ubah \textit{reset password}.
	\end{itemize}
	...
	\begin{itemize}
		\item \verb|update($assignment_id = FALSE)| \\ Memperbarui informasi pada halaman \textit{Detect Similar Codes}.
		\item \verb|_detect($assignment_id = FALSE)| \\ Menjalankan Moss untuk mendeteksi kesamaan kode.
	\end{itemize}
	...
	\begin{itemize}
		...
		\item \verb|edit($notif_id = FALSE)| \\ Memperbarui sebuah notifikasi.
	...
	\begin{itemize}
		\item \verb|index($assignment_id = NULL, $problem_id = 1)| \\ Mengambil data yang diperlukan dan menampilkan halaman \textit{Problems}.
		\item \verb|edit($type = 'md', $assignment_id = NULL, $problem_id = 1)| \\ Memperbarui deskripsi \textit{problem} dan menampilkan halaman \textit{Edit Problem Description}.
	\end{itemize}
	
	\item \verb|Profile| \\ Controller untuk menangani halaman \textit{Profile}. Fungsi yang dimiliki:
	\begin{itemize}
		\item \verb|index($user_id = FALSE)| \\ Mengambil data yang diperlukan dan menampilkan halaman \textit{Profile}.
	...
	\begin{itemize}
        ...
        \item \verb|pause()| \\ Memberhentikan antrian.
        \item \verb|resume()| \\ Melanjutkan antrian.
        \item \verb|empty_queue()| \\ Mengosongkan antrian.
	\end{itemize}
	
	\item \verb|Queueprocess| \\Controller untuk menangani proses penilaian kode. Fungsi yang dimiliki:
	\begin{itemize}
        \item \verb|run()| \\ Menilai kode satu per satu dari antrian.
	\end{itemize}
	...
	\begin{itemize}
        ...
        \item \verb|_upload()| \\ Menyimpan file yang diunggah dan menambahkannya ke dalam antrian.
	\end{itemize}
